...
One-command setup in Termux (installs deps, downloads this script, runs it, adds to bashrc):
  curl -L https://raw.githubusercontent.com/autogit-app/autogit/main/scripts/termux_setup.sh | bash
"""
...
def safe_join(base, *parts):
    """Join and ensure result is under base (no escape)."""
    full = os.path.normpath(os.path.join(base, *parts))
    if not os.path.commonpath([base, full]) == base:
...
def list_repo_dir(repo_name, subpath):
    full = os.path.join(REPOS_DIR, repo_name)
    if not is_git_dir(full):
        raise FileNotFoundError(f"Not a git repository: {repo_name}")
    target = safe_join(full, subpath) if subpath else full
    if not os.path.isdir(target):
        return []
    result = []
    for name in sorted(os.listdir(target)):
        if name == ".git" and not subpath:
            continue
        if name.startswith("."):
            continue
        entry_path = os.path.join(target, name)
        result.append({"name": name, "isDir": os.path.isdir(entry_path)})
    return result


def read_repo_file(repo_name, subpath):
    full = os.path.join(REPOS_DIR, repo_name)
    if not is_git_dir(full):
        raise FileNotFoundError(f"Not a git repository: {repo_name}")
    target = safe_join(full, subpath)
...
def write_repo_file(repo_name, subpath, content):
    full = os.path.join(REPOS_DIR, repo_name)
    if not is_git_dir(full):
        raise FileNotFoundError(f"Not a git repository: {repo_name}")
    target = safe_join(full, subpath)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path.startswith("/repos/") and "/list" in path:
            # /repos/<name>/list?path=...
            parts = path.split("/")
            if len(parts) >= 4 and parts[2]:
                repo_name = unquote(parts[2])
                subpath = (qs.get("path") or [""])[0]
                try:
                    self._json(list_repo_dir(repo_name, subpath))
                ...
                self._error("Missing repo name", 400)
        elif path.startswith("/repos/") and "/read" in path:
            parts = path.split("/")
            if len(parts) >= 4 and parts[2]:
                repo_name = unquote(parts[2])
                subpath = (qs.get("path") or [""])[0]
                if not subpath:
                    ...
                    return
                try:
                    content = read_repo_file(repo_name, subpath)
    ...
    def do_POST(self):
        ...
        elif path.startswith("/repos/") and path.endswith("/write"):
            parts = path.split("/")
            if len(parts) >= 4 and parts[2]:
                repo_name = unquote(parts[2])
                ...
                if not isinstance(content, str):
                    content = str(content)
                try:
                    write_repo_file(repo_name, subpath, content)
